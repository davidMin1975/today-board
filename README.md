...
기술영업 · 사업관리 · 시스템 모니터링 · 개인 루틴을 **한 화면에서** 관리하는 고밀도 다크 대시보드.
...
- **미니멀 다크 UI** — Slate/Zinc 기반, 앰버골드 액센트, 데스크톱 3열 / 모바일 1열 반응형.
...
### 3. 일정 & 타임라인
- 오전 / 오후 / 저녁 시간대별 고정 루틴 · 미팅 타임블록
- **현재 진행 중인 일정 하이라이트** + `NOW` 표시, 항목별 체크
...
- **집중 타이머** — 뽀모도로 25분 집중 / 5분 휴식, SVG 링 게이지, 시작 / 일시정지 / 리셋
  - 세션 종료 시 **Web Audio 비프음** + **브라우저 알림**(선택 토글)
- **일일 루틴 체크리스트** — 수영 · 브리핑 · 물 섭취 등, `오늘 진행 상태 초기화`
